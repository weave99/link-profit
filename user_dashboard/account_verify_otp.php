    ...
    $verify_otp = $_POST['verify_otp'];
        ...
        if ($result->num_rows > 0) {
            ...
            if($r=mysqli_fetch_array($query))
            {
                ...
                mysqli_query($conn,"UPDATE users set account_status='Verified', verify_otp='" . NULL . "' where email='$email'");
              
                header("location:dashboard.php");
            }
            else{
                    header("location:account_verify_otp.php?email=".$email."&msg=Invalide Code");
                }
        } 
        else {
                                ...
                                <h6 class="text-center">One Confirmation code has been send to your email id</h6>
                                    <form action="" method="post">
                                        <div class="form-group">
                                            ...
                                            <input type="number" class="form-control" name="verify_otp" placeholder="Enter Code" required>
                                        </div>
                                        <p class="text-danger"><b><?php if (isset($_REQUEST["msg"])) {$msg = $_REQUEST["msg"];echo $msg ;}?></b></p>
                                        <div class="form-group clearfix">
                                            <button type="submit" class="btn btn-sm btn-success" name="send" style="position: relative; float: right;">Verify</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
